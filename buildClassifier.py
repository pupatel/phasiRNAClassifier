...
##This script builds Random Forest classifier and perfroms stratified 5 fold CV five complete times and outputs accuracy, presicion, recall, roc curve, and AUC.
...
def cross_validaiton(FeatureSet,n_folds,n_trees,n_features,n_jobs,X_new,Feature_labels,clf):
    ...
    for i in range(n_folds): # Run five complete runs of 5-fold cross-valdation to ensure that the results does not depend on one 5-way split for one time 5-fold cross-validation.
        ...
        # Compute Recall 
        recall = cross_val_score(clf, X_new, Feature_labels_converted, cv=skf, scoring='recall')
        RE.append(recall.mean())
        RE_stdev.append(recall.std())

        # Compute Precision 
        precision = cross_val_score(clf, X_new, Feature_labels_converted, cv=skf, scoring='precision')
        PPV.append(precision.mean())
        PPV_stdev.append(precision.std())
        
        # Compute Specificity 
        y_prediction=cross_val_predict(clf,X_new, Feature_labels_converted,cv=skf)
